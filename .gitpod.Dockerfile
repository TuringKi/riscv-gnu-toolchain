...
RUN apt-get update && apt-get install -yq build-essential libgmp3-dev libmpfr-dev libmpc-dev flex bison autogen gcc-multilib dejagnu clangd clang-tidy bear htop vim \
    git \
    git-lfs \
    sudo \
